...
Renderer::Renderer(unsigned int screen_width, unsigned int screen_height, RendererSettings rs)
	...
	m_fov(rs.fov),
	m_model(Model("../assets/objs/teapot.obj")),
...
Pixel* Renderer::Update(float elapsed_time, float cam_forward, float dx, float dy)
{
	...
	m_model = m_model.GetQuaternion() * qx * qy;

	m_model.Translate(0.f, -3.f, 6.f);
	...
	{
		// rotate object
		TransformTriangle(o, m_model.GetQuaternion().convert_to_mat());

		TransformTriangle(o, m_model.GetTransform());
...
void Renderer::reset_object()
{
	m_model = mathz::Quaternion(1.f, 0.f, 0.f, 0.f);
}
